...
public class VideoService {
    ...
    public Boolean verifyUser(String access_token){
        String authUrl = "http://host.docker.internal:8083/api/verify";
    ...
    public String getAuthor(String access_token){
        ...
        String userUrl = "http://host.docker.internal:8083/api/userName/"+email;
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", "Bearer " + access_token);
        HttpEntity<String> req = new HttpEntity<String>(headers);
    ...
    public Long getUserId(String access_token){
        DecodedJWT jwt = JWT.decode(access_token);
        String email = jwt.getSubject();
        String userUrl = "http://host.docker.internal:8083/api/id/"+email;
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", "Bearer " + access_token);
        HttpEntity<String> req = new HttpEntity<String>(headers);
    ...
    public String getFileLink(MultipartFile file){
        String uploadUrl = "http://host.docker.internal:8084/file/upload";
